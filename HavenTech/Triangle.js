...
function TRI_Setup(tri, fTriIdx, face, vertArr ){
	...
	Vect3_CopyFromArr( e1W, vertArr, face.vertIdxs[tri.i2]*vertCard );
	Vect3_SubFromArr( e1W, vertArr, face.vertIdxs[tri.i1]*vertCard ); //from vert1 to vert2
	
	Vect3_CopyFromArr( e2W, vertArr, face.vertIdxs[tri.i3]*vertCard ); //from vert1 to vert3
	Vect3_SubFromArr( e2W, vertArr, face.vertIdxs[tri.i1]*vertCard );
	//use edge1 and edge2 to get a vector perpendicular 
	//to the triangle surface ( the normal )
	
	//world space tri normal
	Vect3_Cross( tri.triZW, e1W, e2W );
	Vect3_Normal( tri.triZW );

	//x and y world space component vectors of the local triangle space
	Vect3_Copy(tri.triXW, e1W); //avoid e1W norm, it's used in v2L_e1L
	Vect3_Normal( tri.triXW );
	
	Vect3_Cross( tri.triYW, tri.triZW, tri.triXW );
	Vect3_Normal( tri.triYW );
	...
	tri.v2L_e1L[0] = Vect3_Dot( e1W, tri.triXW );
	tri.v2L_e1L[1] = Vect3_Dot( e1W, tri.triYW );
	tri.v2L_e1L[2] = Vect3_Dot( e1W, tri.triZW );
	
	//this is v3 in local space and also the vector e2L (dot product coord remap)
	tri.v3L_e2L[0] = Vect3_Dot( e2W, tri.triXW );
	tri.v3L_e2L[1] = Vect3_Dot( e2W, tri.triYW );
	tri.v3L_e2L[2] = Vect3_Dot( e2W, tri.triZW );
	
	//use difference of v3L and v2L to get e3L
	Vect3_Copy( tri.e3L, tri.v3L_e2L ); //from v3Local to v2Local
	Vect3_Subtract( tri.e3L, tri.v2L_e1L );
	
	//get lengths of local edges
	tri.e1LenSq = Vect3_LengthSquared( tri.v2L_e1L );
	tri.e2LenSq = Vect3_LengthSquared( tri.v3L_e2L );
	tri.e3LenSq = Vect3_LengthSquared( tri.e3L );
	
	//normalize edges
	//Vect3_Normal( tri.v2L_e1L );
	//Vect3_Normal( tri.v3L_e2L );
	//Vect3_Normal( tri.e3L );
	
	
	//generate orthogonals of triangle edges facing outwards
	tri.e1LOrtOut[0] =  tri.v2L_e1L[1];
	tri.e1LOrtOut[1] = -tri.v2L_e1L[0]; //clockwise
	
	tri.e2LOrtOut[0] = -tri.v3L_e2L[1];
	tri.e2LOrtOut[1] =  tri.v3L_e2L[0]; //counterclockwise
	
	tri.e3LOrtOut[0] =  tri.e3L    [1];
	tri.e3LOrtOut[1] = -tri.e3L    [0]; //clockwise
...
let e2OrthogDotL;
let e3OrthogDotL;
function TRI_RayIntersection(tri, ray, vArr, v1WI )
{
	...
	rayNormLZ =  Vect3_Dot( ray.norm, tri.triZW );
	...
	rayOriL[0] = Vect3_Dot( triToRayOriW, tri.triXW );
	rayOriL[1] = Vect3_Dot( triToRayOriW, tri.triYW );
	rayOriL[2] = Vect3_Dot( triToRayOriW, tri.triZW );

	rayNormL[0] = Vect3_Dot( ray.norm, tri.triXW );
	rayNormL[1] = Vect3_Dot( ray.norm, tri.triYW );
	...
	RayPointAtTime( pointL, rayOriL, rayNormL, rayDistToPtWL);
	...
	//e1OrthogDotL =  Vect_Dot( tri.e1LOrtOut, tri.pointL ); 
	//e1Orthog, the vector from v1L to v2L is rotated clockwise
	//to face outwards from the triangle

	//e2OrthogDotL = Vect_Dot( tri.e2LOrtOut, tri.pointL );
	//the vector from v1L to v3L 
	//is rotated counterclockwise to face outwards

	//vToPtFromV2L[0] = tri.pointL[0] - (tri.v2L_e1L[0]);// * tri.e1Len);
	//vToPtFromV2L[1] = tri.pointL[1] - (tri.v2L_e1L[1]);// * tri.e1Len);
	//vector to the point from (v2L) which is on edge3L (from v3L to v2L)
	
	//e3OrthogDotL =  Vect_Dot( tri.e3LOrtOut, vToPtFromV2L );
	...
	baryVertPcts[2] = R2RayIntersection( tri.v1L,     tri.v2L_e1L, pointL, tri.e1LOrtOut ) * tri.e1LenSq;
	baryVertPcts[1] = R2RayIntersection( tri.v1L,     tri.v3L_e2L, pointL, tri.e2LOrtOut ) * tri.e2LenSq;
	baryVertPcts[0] = R2RayIntersection( tri.v2L_e1L, tri.e3L,     pointL, tri.e3LOrtOut ) * tri.e3LenSq;
...
let baryVertPcts = new Float32Array(3);
let baryAreaSum = 0;
function TRI_UVCoordOfPoint( tri, uv, face ){
	...
	baryAreaSum = baryVertPcts[0]+baryVertPcts[1]+baryVertPcts[2];
	...
	*/
	...
	uv[1] = 0;//v3Pct;
	//return;
	VectArr_PctAdd( uv, face.uvs, tri.i1*uvCard, baryVertPcts[0] / baryAreaSum );
	VectArr_PctAdd( uv, face.uvs, tri.i2*uvCard, baryVertPcts[1] / baryAreaSum );
	VectArr_PctAdd( uv, face.uvs, tri.i3*uvCard, baryVertPcts[2] / baryAreaSum );
