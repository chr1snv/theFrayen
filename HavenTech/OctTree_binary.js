...
function TreeNode( axis, minCoord, maxCoord, parent ){
	...
	this.addToThisNode = function( object ){
		...
		for( let i = 0; i < this.objects.length; ++i ){
			let aObjMin = this.objects[i].AABB.minCoord;
			let objMin  = object.AABB.minCoord;
			//sort objects by all three axies
			if(  //(aObjMin[this.axis] < objMin[this.axis] ) ||
				 aObjMin[0] < objMin[0] || 
				  aObjMin[1] < objMin[1] ||
				  aObjMin[2] < objMin[2]  ){
	...
	this.updateParentLeafCountsAndRebalanceIfNecessary = function( ovAxis ){
		...
		while( Math.abs(parLeavDiff) < 0.35 ){
			...
			rebalanceParent.leaveCounts[0] = rebalanceParent.minNode.leaveCounts[0]+rebalanceParent.MaxNode.leaveCounts[0];
			rebalanceParent.leaveCounts[1] = rebalanceParent.minNode.leaveCounts[1]+rebalanceParent.MaxNode.leaveCounts[1];
			rebalanceParent.leaveCounts[2] = rebalanceParent.minNode.leaveCounts[2]+rebalanceParent.MaxNode.leaveCounts[2];
		...
		if(rebalanceParent != null){ //found a node that needs rebalancing
			...
			if( rebalanceParent.minNode.minNode != null )
				minMsg += rebalanceParent.minNode.minNode.leaveCounts + " : " + rebalanceParent.minNode.MaxNode.leaveCounts;
			let maxMsg = " maxLfs ";
			if( rebalanceParent.MaxNode.minNode != null )
				maxMsg += rebalanceParent.MaxNode.minNode.leaveCounts + " : " + rebalanceParent.MaxNode.MaxNode.leaveCounts;
			console.log( "%crebal at dpth " + rebalanceParent.depth + 
						...
						" leaveCts " + rebalanceParent.leaveCounts +
						 " minAx " + rebalanceParent.minNode.axis + 
						 minMsg +
						 " :: maxAx " + rebalanceParent.MaxNode.axis + 
						 maxMsg, "color:lightblue" );
	...
	this.DebugDraw = function(fCtx){
		...
		if( this.objects.length > 0 ){
			...
			let txt = nodeNum+":"+this.objects.length+minMax; //":"+this.AABB.minCoord[0]+",\n"+this.AABB.minCoord[1]+
			...
			//draw number on each node
			for( var i = 0; i < this.objects.length; ++i ){
				let cent = Vect3_CopyNew( this.objects[i].AABB.minCoord );
				Vect3_Add( cent, this.objects[i].AABB.maxCoord );
