...
//function members
    ...
    this.GetLight = function(lightName){}
    this.GetActiveCamera = function() { return this.cameras[activeCameraIdx]; }

    this.GetSceneGraph = function(){return sceneGraph; }
    this.GetHitSceneGraph = function() { return hitSceneGraph; }

    var parseSceneFile = function(textFileLines, thisP){
        var discard = '0';
        var pos = new Array(3);
        var rot = new Array(3);
        var col = new Array(3);
        ...
        for(var i=0; i<textFileLines.length; ++i)
        {
            ...
            if(temp[0] == 'c'){
                ...
                thisP.cameras.push_back(new Camera(cameraName, sceneName, angle, clipStart, clipEnd, pos, rot));
            }
            ...
            if(temp[0] == 'a' && temp[1] == 'c'){
                ...
                for(var i=0; i<cameras.size(); ++i)
                    if(cameras[i].GetName() == activeCamera)
                        thisP.activeCameraIdx = i;
            }
        }
        thisP.isValid = true;
        ...
        txtFile.onreadystatechange = function(){
            if(txtFile.readyState === 4){
                if(txtFile.status === 200){
                    ...
                    var linesArray = txtFile.responseText.split("\n");
                    callback(linesArray, thisP); //callback
                }
                else
